...
# expensive outcomes include aligning at position 1 (41 fuel), position 3 (39 fuel), 
# or position 10 (71 fuel).

# Determine the horizontal position that the crabs can align to using the least 
# fuel possible. How much fuel must they spend to align to that position?

path_data = "./day_07.txt"
...
for i in range(min_val, max_val):
    # x = sum([abs(j-i) for j in input]) # part 1
    x = sum([fuel_cost(abs(j-i)) for j in input]) # part 2
    min_fuel = min([min_fuel, x])
print(min_fuel)
...
# best horizontal position to align them all on; in the example above, 
# this becomes 5:
